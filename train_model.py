...
class KeyPointGaussian(nn.Module):
    ...
    def forward(self, x):
        ...
        if gpu_enabled == True:
            ...
            linh = torch.linspace(0, self.h - 1, self.h).view(1, 1, self.h, 1).expand([1, 1, self.h, self.w])
            linw = torch.linspace(0, self.w - 1, self.w).view(1, 1, 1, self.w).expand([1, 1, self.h, self.w])
